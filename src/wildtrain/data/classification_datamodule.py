    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit":
            ...
            self.val_dataset = load_all_splits_concatenated(
                self.root_data_directory, splits=["val"], transform=self.transforms, **self.single_class_config
            )["val"]
            self.class_mapping = self._get_class_mapping(self.val_dataset)
        elif stage == "test":
            self.test_dataset = load_all_splits_concatenated(
                self.root_data_directory, splits=["test"], transform=self.transforms, **self.single_class_config
            )["test"]
