...
def localizer(
    config: Path = typer.Option(None, "--config", "-c", help="Path to detector registration configuration file"),
    ...
    model_type:str = typer.Option("yolo", "--type", help="localizer model type"),
    template: bool = typer.Option(False, "--template",help="Show default configuration template instead of registration")
) -> None:
    """Register a YOLO or MMDet detection model to MLflow Model Registry."""
    if template:
    ...
    if config is not None:
        ...
        try:
            validated_config = ConfigLoader.load_localizer_registration_config(config)
            ...
            dynamic = validated_config.processing.dynamic
            export_format = validated_config.processing.export_format
            mlflow_tracking_uri = validated_config.processing.mlflow_tracking_uri
            batch_size = validated_config.processing.batch_size
            
            if validated_config.yolo:
                weights_path = Path(validated_config.yolo.weights)
                image_size = validated_config.yolo.imgsz
                device = validated_config.yolo.device
                task = validated_config.yolo.task
            else:
                raise NotImplementedError("Only Yolo is supported. provide arguments")
